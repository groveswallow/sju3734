...
@LastEditors: Tang
@LastEditTime: 2019-12-03 19:00:30
@Description: 未改写成功。原因不知道tfcountclass中的每列含义。
'''
import openpyxl
import sys
import os
...
def add(path):
    ...
    for i in file_dir:
        ...
        cresql = """CREATE TABLE %s(
            tf CHAR(10) not null,number INT(10),tfclass CHAR(100));""" % i[0:-5]
        cur.execute(cresql)
        print("create %s success\n"%i[0:-5])
        for x in range(1, ws.max_row+1):
            if ws['A%d' % x].value == None:
                break
            if ws['B%d' % x].value == None:
                num = 0
            else:
                num = int(ws['B%d' % x].value)
            if ws['c%d' % x].value == None:
                tfclass = 'unkown'
            else:
                tfclass =  ws['c%d' % x].value         
            insql = """INSERT INTO lcbb3734.%s
            VALUES ('%s',%d,'%s');""" % (i[0:-5],ws['A%d' % x].value,num,tfclass)
            print(insql)
